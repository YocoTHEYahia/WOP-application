...
export const tripStore = {
  ...
  saveActiveTrip: (trip: ActiveTrip | null) =>
    trip
  ...
  saveHistory: (history: TripHistoryEntry[]) =>
    AsyncStorage.setItem(KEYS.history, JSON.stringify(history.slice(0, 50))),
  getDestinations: () => readJson<Destination[]>(KEYS.destinations, []),
  saveDestinations: (destinations: Destination[]) =>
    AsyncStorage.setItem(
      KEYS.destinations,
      JSON.stringify(destinations.slice(0, 50)),
    ),
  getSettings: () => readJson<WopSettings>(KEYS.settings, DEFAULT_SETTINGS),
  saveSettings: (settings: WopSettings) =>
